...
module {
  ...
  func sort<T>(
    ...
    if (fullArray) {
      ...
      for (item in array.vals()) {
        let index = nat64ToNat(from64 +% scale *% nat64(nat16ToNat32(map(item) -% min)) / step);
      ...
      loop {
        let index = nat64ToNat(from64 +% scale *% nat64(nat16ToNat32(map(array[nat(i)]) -% min)) / step);
