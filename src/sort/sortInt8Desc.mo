import Prim "mo:prim";
import { int8ToNat8; nat8ToNat16; nat16ToNat32; nat64ToNat; intToNat32Wrap; nat32ToNat = nat; nat32ToNat64 = nat64 } "mo:prim";
...
module {
  ...
  func sort<T>(
    ...
    if (fullArray) {
      ...
      let minMaxDiff = nat64(nat16ToNat32(nat8ToNat16(int8ToNat8(max -% min))));
      let scale = 0xffffffffffffffff / minMaxDiff;
      let step = scale *% minMaxDiff / nat64(to -% from);
      let to64 = nat64(to);

      for (item in array.vals()) {
        let index = nat64ToNat(to64 -% scale *% nat64(nat16ToNat32(nat8ToNat16(int8ToNat8(map(item) -% min)))) / step);
      ...
      loop {
        let index = nat64ToNat(to64 -% scale *% nat64(nat16ToNat32(nat8ToNat16(int8ToNat8(map(array[nat(i)]) -% min)))) / step);
