...
module {
  func sort<T>(
    ...
    while (i <= to) {
      let index = nat64ToNat(to64 -% scale *% nat64(nat8ToNat(int8ToNat8(map(array[nat(i)]) -% min))) / step);
    ...
    while (i <= to) {
      let iNat = nat(i);
      let index = nat64ToNat(to64 -% scale *% nat64(nat8ToNat(int8ToNat8(map(array[iNat]) -% min))) / step);
