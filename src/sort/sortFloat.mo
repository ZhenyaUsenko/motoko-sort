...
module {
  ...
  func sort<T>(
    ...
    if (fullArray) {
      ...
      let fromFloat = int64ToFloat(nat64ToInt64(nat64(from)));

      for (item in array.vals()) {
        let index = nat64ToNat(int64ToNat64(floatToInt64(fromFloat + (map(item) - min) / step)));
      ...
      loop {
        let index = nat64ToNat(int64ToNat64(floatToInt64(fromFloat + (map(array[nat(i)]) - min) / step)));
