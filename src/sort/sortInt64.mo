import { Array_init = initArray; nat32ToNat = nat; int64ToNat64; nat64ToNat; intToNat32Wrap = nat32; intToNat64Wrap = nat64 } "mo:prim";
...
module {
  func sort<T>(
    ...
    while (i <= to) {
      let iNat = nat(i);
      let index = nat64ToNat(from64 +% scale *% int64ToNat64(map(array[iNat]) -% min) / step);
