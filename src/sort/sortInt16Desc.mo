...
module {
  func sort<T>(
    ...
    while (i <= to) {
      let index = nat64ToNat(to64 -% scale *% nat64(nat16ToNat(int16ToNat16(map(array[nat(i)]) -% min))) / step);
    ...
    while (i <= to) {
      let iNat = nat(i);
      let index = nat64ToNat(to64 -% scale *% nat64(nat16ToNat(int16ToNat16(map(array[iNat]) -% min))) / step);
