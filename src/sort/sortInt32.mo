import Prim "mo:prim";
import { int32ToNat32; nat64ToNat; intToNat32Wrap; nat32ToNat = nat; nat32ToNat64 = nat64 } "mo:prim";
...
module {
  ...
  func sort<T>(
    ...
    if (fullArray) {
      ...
      let minMaxDiff = nat64(int32ToNat32(max -% min));
      let scale = 0xffffffffffffffff / minMaxDiff;
      let step = scale *% minMaxDiff / nat64(to -% from);
      let from64 = nat64(from);

      for (item in array.vals()) {
        let index = nat64ToNat(from64 +% scale *% nat64(int32ToNat32(map(item) -% min)) / step);
      ...
      loop {
        let index = nat64ToNat(from64 +% scale *% nat64(int32ToNat32(map(array[nat(i)]) -% min)) / step);
