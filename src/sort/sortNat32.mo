import Prim "mo:prim";
import { nat64ToNat; intToNat32Wrap; nat32ToNat = nat; nat32ToNat64 = nat64 } "mo:prim";
...
module {
  ...
  func sort<T>(
    ...
    if (fullArray) {
      ...
      loop {
        let index = nat64ToNat(from64 +% scale *% nat64(map(array[nat(i)]) -% min) / step);
