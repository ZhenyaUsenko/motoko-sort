...
module {
  ...
  func sort<T>(
    ...
    if (fullArray) {
      ...
      let from64 = nat64(from);

      for (item in array.vals()) {
        let index = nat64ToNat(from64 +% scale *% nat64(nat16ToNat32(nat8ToNat16(int8ToNat8(map(item) -% min)))) / step);
      ...
      loop {
        let index = nat64ToNat(from64 +% scale *% nat64(nat16ToNat32(nat8ToNat16(int8ToNat8(map(array[nat(i)]) -% min)))) / step);
