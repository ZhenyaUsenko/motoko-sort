...
module {
  func sort<T>(
    ...
    while (i <= to) {
      let index = nat64ToNat(to64 -% scale *% int64ToNat64(map(array[nat(i)]) -% min) / step);
    ...
    while (i <= to) {
      let iNat = nat(i);
      let index = nat64ToNat(to64 -% scale *% int64ToNat64(map(array[iNat]) -% min) / step);
