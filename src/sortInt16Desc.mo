import { Array_init = initArray; int16ToInt; nat32ToNat = nat; nat64ToNat; intToNat32Wrap = nat32; intToNat64Wrap = nat64 } "mo:prim";
...
module {
  func sort<T>(
    ...
    let minNat64 = nat64(int16ToInt(min));
    let minMaxDiff = nat64(int16ToInt(max)) -% minNat64;
    ...
    while (i <= to) {
      let iNat = nat(i);
      let index = nat64ToNat(to64 -% scale *% (nat64(int16ToInt(map(array[iNat]))) -% minNat64) / step);
