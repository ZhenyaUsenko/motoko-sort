...
6) for each `count` from the `range` in the `counts` array which is not `0`
    - set `shiftedCounts` array at index `totalCount` to `count`
...
9) for each `index` and `count` from the `range` in the `shiftedCounts` array where `count` is greater than `1` run steps `2-9` with the `range` set to `[index, index + count - 1]`
